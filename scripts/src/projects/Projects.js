...
let projects = [
	{
		id: '1',
		image: './assets/img/keepquran.png',
		title: 'KeepQuran',
		link: 'https://keepquran.web.app/',
		langs: ['javascript'],
	},
	{
		id: '2',
		image: './assets/img/orgman.png',
		title: 'Orgman',
		link: 'https://orgman.web.app/',
...
const FilterProjects = (filterLang = false) => {
	...
	const filterdProjects = projects.filter((project) =>
		project.langs.includes(filterLang)
	);
	...
	filterdProjects.forEach((project, idx) => {
		...
		if (!found) {
			...
			ProjectsContainer.insertBefore(
				newProject,
				ProjectsContainer.children[idx]
			);
			FoundArray.push(idx);
		}
	});
	...
	while (removingCounter >= 0) {
		if (!FoundArray.includes(removingCounter)) {
			ProjectsContainer.removeChild(
				ProjectsContainer.children[removingCounter]
			);
		}
